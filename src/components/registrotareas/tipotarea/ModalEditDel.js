...
const Modaleditdel = (props) => {
    const [show, setShow] = useState(false);
    const numero = props.tryId
    const handleClose = () => setShow(false);

    const handleShow = () => {
        setShow(true)
        //verificamos si existe una tarea seleccionada con el prop que lee el id de tarea
        if (numero === '#') {
            console.log('hola no hay id');
        } else {
            console.log('ahora hay un id');
            // valores por defecto en los combos
            const tareaEdEl = alltareas[numero].id
            const datatareaEdEl = alltareas[numero]
            console.log(tareaEdEl, datatareaEdEl)

            if(datatareaEdEl.tipo_tarea_id===1){

            }

            switch (datatareaEdEl.tipo_tarea_id) {
                case 1:{
    ...
    //     switch (opTarea) {
    //         case 'Incidencia':
    //             return <IncidenteDelEdit />
    //         case 'Plan de Accion':
    //             return <PlanaccionDelEdit />
    //         case 'Pase':
    //             return <PaseDelEdit />
    //         case 'Otro':
    //             return <OtroDelEdit />
    ...
    return (
        <>
            <Button variant="primary" onClick={handleShow}>
                Ver Detalle Tarea {numero}
            </Button>

            <Modal show={show} onHide={handleClose}>
                <Modal.Header closeButton>
                    <Modal.Title>Tarea numero {numero}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                        ...
                        <div className="card card-primary">
                            ...
                            <form  >
                                ...
                                <div className="card-footer">
                                    <button className="btn btn-primary"   >Editar Registro</button>
                                    <button className="btn btn-danger" data-dismiss="modal" onClick={enviarDatos} >Eliminar Registro X</button>
                                </div>
                            </form>
                        </div>
                    </div>
